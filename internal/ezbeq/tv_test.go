...
// the catalog entries for Silo, verbatim, because authors are not consistent
// about where they put the episodes
func TestMatchTVEntryAgainstRealCatalog(t *testing.T) {
	silo := []models.BeqCatalog{
...
func TestBuildSearchEndpoint(t *testing.T) {
	t.Run("movie is filtered by year", func(t *testing.T) {
		m := &models.SearchRequest{MediaType: "Movie", Codec: "DD+ Atmos", Year: 2016, TMDB: "335984"}
		assert.Equal(t, "/api/1/search?audiotypes=DD%2B+Atmos&years=2016&tmdbid=335984", buildSearchEndpoint(m))
	})

	t.Run("episode is not filtered by year", func(t *testing.T) {
		m := &models.SearchRequest{MediaType: "Episode", Codec: "DD+ Atmos", Year: 2026, TMDB: "125988", Season: 3, Episode: 1}
		assert.Equal(t, "/api/1/search?audiotypes=DD%2B+Atmos&tmdbid=125988", buildSearchEndpoint(m))
	})

	t.Run("episode without numbering keeps the year", func(t *testing.T) {
		m := &models.SearchRequest{MediaType: "Episode", Codec: "DD+ Atmos", Year: 2026, TMDB: "125988"}
		assert.Equal(t, "/api/1/search?audiotypes=DD%2B+Atmos&years=2026&tmdbid=125988", buildSearchEndpoint(m))
	})
}
